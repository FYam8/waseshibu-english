...
const detailSourceMeta={"2019:6-5":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2020:5-4":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2020:6-1":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2020:6-2":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2021:5-1":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2021:5-2":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2021:5-3":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2021:5-4":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2021:5-6":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2021:6-2":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2021:6-3":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2021:6-4":{"targetId":"detail-context-evidence","focusTag":"connector-context"},"2021:6-6":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2021:6-7":{"targetId":"detail-insertion-cohesion","focusTag":"insertion-cohesion"},"2022:5-1":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2022:5-2":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2022:5-3":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2022:5-5":{"targetId":"detail-causal-inference","focusTag":"action-intent"},"2022:5-6":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2022:6-1":{"targetId":"detail-paraphrase-evidence","focusTag":"study-finding"},"2022:6-2":{"targetId":"detail-context-evidence","focusTag":"connector-context"},"2022:6-4":{"targetId":"detail-paraphrase-evidence","focusTag":"negative-detail"},"2023:5-1":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2023:5-2":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2023:5-3":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2023:5-4":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2023:5-6":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2023:6-2":{"targetId":"detail-context-evidence","focusTag":"connector-context"},"2023:6-4":{"targetId":"detail-context-evidence","focusTag":"vocab-in-context"},"2023:6-5":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2023:6-6":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2023:6-7":{"targetId":"detail-insertion-cohesion","focusTag":"insertion-cohesion"},"2024:5-1":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2024:5-2":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2024:5-3":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2024:5-4":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2024:6-1":{"targetId":"detail-paraphrase-evidence","focusTag":"example-identification"},"2024:6-3":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2025:5-2":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2025:5-3":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2025:5-4":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2025:5-5":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2025:6-2":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2026:7-1":{"targetId":"detail-context-evidence","focusTag":"context-dialogue"},"2026:7-2":{"targetId":"detail-context-evidence","focusTag":"context-word-fit"},"2026:7-3":{"targetId":"detail-causal-inference","focusTag":"reason-motive"},"2026:7-5":{"targetId":"detail-causal-inference","focusTag":"situation-reaction"},"2026:8-3":{"targetId":"detail-paraphrase-evidence","focusTag":"negative-detail"},"2026:8-4":{"targetId":"detail-paraphrase-evidence","focusTag":"paraphrase-scope"},"2026:8-5":{"targetId":"detail-insertion-cohesion","focusTag":"insertion-cohesion"}};
const trapBySkill={
...
function actualMeta(year,q){
 const detailMeta=q.skill==="detail"?detailSourceMeta[`${year}:${q.id}`]:null;
 const focus=q.skill==="reorder"?(reorderFocus[`${year}:${q.id}`]||"fixed-pattern"):
   q.skill==="detail"?(detailMeta?.focusTag||(q.priority==="A"?"explicit-evidence":"inference-evidence")):
 ...
 const target=q.skill==="reorder"?`reorder-${focus}`:
   q.skill==="detail"?(detailMeta?.targetId||targetBySkill.detail):
   (targetBySkill[q.skill]||q.skill);
 return {targetId:target,focusTag:focus,examFormat:q.type,trap:trapBySkill[q.skill]||q.category};
}
...
// detail audit v2: reuse only nd items whose required processing matches an observed source cluster.
for(const id of ["nd01","nd07","nd09"])patch(id,{retired:true,legacyCompletion:true,retiredReason:"detail監査v2: 元detail設問群に対応する中心処理がなく、state-change型として不一致"});
patch("nd02",{targetId:"detail-causal-inference",focusTag:"reason-motive",prompt:"【オリジナル類題】The experiment was repeated three times. The first two results were similar, but the third was much higher because the room had become warmer. Why was the third result different?",explanation:"【正解】The room temperature changed. 【根拠】本文が because the room had become warmer と第三の結果だけ高くなった直接原因を示す。 【他選択肢】equipment、first test、参加人数について原因としての記述はない。 【弱点】理由・原因。because以下を結果と対応させる。 【戦略】A。"});
patch("nd04",{targetId:"detail-causal-inference",focusTag:"reason-motive",prompt:"【オリジナル類題】At first, Amir believed the old map was useless. After comparing it with a photograph, he noticed that both showed the same unusual tree and decided to follow the map. What changed his mind?",explanation:"【正解】A matching detail in a photograph. 【根拠】写真と地図の両方に同じ珍しい木があると気づいた直後に decided to follow the map と判断が変化する。 【他選択肢】友人の警告・地図の古さ・天候は判断変更の原因ではない。 【弱点】理由。考えが変わる直前の根拠を取る。 【戦略】A。"});
patch("nd06",{targetId:"detail-causal-inference",focusTag:"reason-motive",prompt:"【オリジナル類題】Maya did not reply to the message that evening. Her phone had stopped working, and she did not see the message until the next morning. Why was her reply late?",explanation:"【正解】Her phone was not working. 【根拠】phone had stopped working → did not see the message until the next morning。返信が遅れた直接原因が明示されている。 【他選択肢】意見の不一致・住所紛失・電車移動は本文にない。 【弱点】理由。結果と直接原因を対応させる。 【戦略】A。"});
patch("nd03",{targetId:"detail-paraphrase-evidence",focusTag:"paraphrase-scope",prompt:"【オリジナル類題】Most of the 30 students chose the library, while six preferred the gym. Which statement is correct?",explanation:"【正解】More students chose the library than the gym. 【根拠】30人中gymは6人で、mostはlibraryを選んだ。mostはallではない。 【他選択肢】all、libraryが6人、gymが0人はいずれも本文と矛盾。 【弱点】内容説明。most/allや人数関係を正確に言い換える。 【戦略】A。"});
patch("nd05",{targetId:"detail-paraphrase-evidence",focusTag:"paraphrase-scope",prompt:"【オリジナル類題】The new machine finished the work quickly, but every result still had to be checked by an experienced worker. What limitation remained?",explanation:"【正解】Its results required human checking. 【根拠】but以下の every result still had to be checked by an experienced worker が残った制約。 【他選択肢】紙の量・速度低下・起動不能は本文にない。 【弱点】内容説明。but後の制約を言い換える。 【戦略】A。"});
patch("nd08",{targetId:"detail-paraphrase-evidence",focusTag:"study-finding",prompt:"【オリジナル類題】Although the medicine reduced pain for many adult patients, the report says that it has not yet been tested on children. What can be concluded?",explanation:"【正解】Its effect on children is still unknown. 【根拠】childrenには not yet been tested。未実施の対象について安全性や効果を断定できない。 【他選択肢】全児童に安全、成人に効かなかった、子供だけを調べた、はいずれも本文と矛盾。 【弱点】調査結果の範囲。調べた対象と未調査対象を分ける。 【戦略】B。"});
patch("nd10",{targetId:"detail-paraphrase-evidence",focusTag:"negative-detail",prompt:"【オリジナル類題】The article says that planting trees near classrooms has three benefits: the trees block strong sunlight, keep rooms cooler, and reduce the electricity used for air conditioning. Which benefit is NOT stated?",explanation:"【正解】Quieter traffic. 【根拠】本文には less direct sunlight、cooler rooms、lower electricity use の3点が明記されているが、traffic noise は述べられていない。 【他選択肢】残り3つは本文の表現をそのまま、またはほぼ同じ意味で言い換えたもの。 【弱点】照合・除外。NOTを確認し、各選択肢を本文と1対1で照合する。 【戦略】A。"});

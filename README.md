...
2019〜2026年度の自由記述8問と、その克服ドリル45問（英文完成14・要約15・要約＋反論16）では、任意で英文答案を入力し、内容・論理・文法・綴りの具体的な改善点を確認できます。正答が一意に決まる5点の英文完成問題は、従来どおりルール採点を使います。

- 表示点は公式の詳細配点ではなく、固定ルールで算出する「学習用AI採点（12点または24点相当）」です。
- AIは意味要素と最大3件の具体的指摘を返し、点数計算はコード側で行います。
- `First` / `Second` など特定の接続語や、具体例がないという事実だけでは減点しません。文章全体のつながりと、理由の具体性を見ます。
- 採点はボタンを押したときだけ1回実行し、自動再試行しません。答案は最大1,200文字、AI出力は最大900 tokenです。
- 自己採点は引き続き利用でき、AIの点数は「反映」ボタンを押すまで自己採点欄を上書きしません。
- 類題でもAIの具体的指摘を確認できますが、従来の答案例・必須チェック・自己判定は残しています。
- 既存の保存キー `waseshibu.adaptive.v3` と schema version 8 は変更せず、AI結果を記述答案オブジェクトの追加フィールドとして保存します。

...
export async function runAnswerWidgetChecks(browser,url,isRikkyo){
 for(const width of [390,1280]){
  ...
  if(isRikkyo){
   ...
   await row.locator('.tokens button').nth(0).click();assert.equal(await row.locator('.tokens button').nth(0).isDisabled(),true);
   await row.locator('.tokens button').nth(1).click();
   await row.getByRole('button',{name:'1語戻す',exact:true}).click();assert.deepEqual((await read()).order,[0]);
   await row.locator('.missing-word-row input').fill('had');await row.getByRole('button',{name:'不足語を追加',exact:true}).click();
   ...
   await row.getByRole('button',{name:'やり直す',exact:true}).click();assert.deepEqual((await read()).order,[]);assert.equal((await read()).missing,'edited');
   ...
   const tokens=p.locator('.drill-card .tokens button');await tokens.nth(1).click();await tokens.nth(2).click();assert.equal(await p.locator('#orderBox').innerText(),'had had');
   await p.getByRole('button',{name:'1語戻す',exact:true}).click();assert.equal(await p.locator('#orderBox').innerText(),'had');
   ...
   await p.getByRole('button',{name:'やり直す',exact:true}).click();assert.equal(await p.locator('#orderBox').innerText(),'');

...
assert.match(prefix,/GOAL_TIERS=Array\.isArray\(SCHOOL_EXAM_CONFIG\?\.goalTiers\)\?\[\.\.\.SCHOOL_EXAM_CONFIG\.goalTiers\]:\[60,70,75\]/);
...
assert.match(app,/function setGoal\(goal\)\{goal=Number\(goal\);if\(!GOAL_TIERS\.includes\(goal\)\)return;/);
assert.doesNotMatch(app,/\[60,70,75\]\.includes\(goal\)/);
assert.doesNotMatch(app,/\[60,70,75\]\.map\(/);

const fakeExam={route:[2030,2031],dailyTaskTarget:7,goalTiers:[55,65],defaultGoal:65,defaultYear:2030};
...
vm.runInContext(`${prefix}\nglobalThis.__runtime={route:ROUTE,daily:DAILY_TASK_TARGET,tiers:GOAL_TIERS,goal:DEFAULT_GOAL,year:DEFAULT_YEAR,initGoal:INIT.goal,initYear:INIT.year}`,ctx);
assert.deepEqual(JSON.parse(JSON.stringify(ctx.__runtime.route)),fakeExam.route);
assert.deepEqual(JSON.parse(JSON.stringify(ctx.__runtime.tiers)),fakeExam.goalTiers);
assert.equal(ctx.__runtime.daily,7);
assert.equal(ctx.__runtime.goal,65);
assert.equal(ctx.__runtime.year,2030);
assert.equal(ctx.__runtime.initGoal,65);
...
vm.runInContext(`${prefix}\nglobalThis.__runtime={route:ROUTE,daily:DAILY_TASK_TARGET,tiers:GOAL_TIERS,goal:DEFAULT_GOAL,year:DEFAULT_YEAR}`,fallbackCtx);
assert.deepEqual(JSON.parse(JSON.stringify(fallbackCtx.__runtime.route)),[2024,2023,2022,2021,2020,2019,2025,2026]);
assert.deepEqual(JSON.parse(JSON.stringify(fallbackCtx.__runtime.tiers)),[60,70,75]);

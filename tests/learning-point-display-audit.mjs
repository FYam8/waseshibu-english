...
assert.equal(releaseIds.length,116)
assert.equal(new Set(releaseIds).size,116)
assert.ok(releaseIds.every(id=>byId[id]),'release scope must contain active questions only')
const releaseSkills=new Set(releaseIds.map(id=>byId[id].skill))
assert.deepEqual([...releaseSkills].sort(),['content_match','emotion','insertion','paraphrase','reason','rebuttal','reorder','sentence_completion'])
for(const id of releaseIds){
  assert.ok(byId[id].point.length>=20,`${id}: released learning point is too short`)
  assert.ok(byId[id].point.length<=180,`${id}: released learning point is too long`)
}
for(const id of ['lrf09','lco22','lcx01','lcx10','lex21','let02','lwc29','lwc31'])assert.equal(releaseIds.includes(id),false,`${id}: HOLD item entered release scope`)
assert.match(byId.nr_iq1.point,/why he was late/)
assert.match(byId.lrb01.point,/宿題全面廃止/)
assert.match(byId.lem01.point,/ジャケット|袖|染み|申し訳/)
assert.match(byId.lpa01.point,/lure.*attract/)

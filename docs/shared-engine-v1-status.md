...
`tests/shared-engine-core-parity.test.mjs` compares these helpers with the current Waseda runtime. `engine/manifest.json` identifies this stage as `shadow-core-parity`, keeps `productionWiring: false`, uses consumer policy `pinned-vendor-pr-only`, and requires `waseda-parity-before-consumer-sync`.

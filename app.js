...
const AI_GRADING_QUESTIONS=new Set(["2024:4","2026:6"]);
...
function rememberAIAnswer(y,id,value){const key=k(y,id),old=S.manual[key]||{score:"",components:[]},fingerprint=aiAnswerFingerprint(value);S.manual[key]={...old,answerText:value,aiFeedbackStale:!!old.aiFeedback&&old.aiFeedback.answerFingerprint!==fingerprint};save();const count=document.getElementById(`ai-word-count-${y}-${id}`);if(count)count.textContent=`${wordCount(value)}語 / 目安50語`}
function aiFeedbackMarkup(y,id,entry){const feedback=entry?.aiFeedback;if(!feedback)return "";const stale=entry.aiFeedbackStale||feedback.answerFingerprint!==aiAnswerFingerprint(entry.answerText||"");const reasons=(feedback.reasons||[]).map(x=>`<li>${h(x)}</li>`).join("");const breakdown=(feedback.breakdown||[]).map(x=>`<div><span>${h(x.name)}</span><b>${Number(x.earned)||0}/${Number(x.max)||0}</b></div>`).join("");const issues=(feedback.issues||[]).map((x,i)=>`<article class="ai-issue ${x.severity==="must_fix"?"must-fix":"improve"}"><div><b>${i+1}. ${h(x.category)}</b><span>${x.severity==="must_fix"?"優先して直す":"さらに良くする"}</span></div><p><del>${h(x.original)}</del>${x.correction?` → <ins>${h(x.correction)}</ins>`:""}</p><p>${h(x.explanationJa)}</p></article>`).join("");const strengths=(feedback.strengths||[]).map(x=>`<li>${h(x)}</li>`).join("");return `<section class="ai-feedback ${stale?"stale":""}"><div class=ai-score-head><div><span>${h(feedback.scoreLabel||"学習用AI採点（24点相当）")}</span><strong>${Number(feedback.score)||0}<small>/24</small></strong></div><span>${Number(feedback.wordCount)||0}語</span></div>${stale?`<p class=ai-stale>答案を変更したため、下の結果は変更前のものです。もう一度採点してください。</p>`:""}<div class=ai-breakdown>${breakdown}</div>${reasons?`<div class=ai-reasons><b>この点数になった理由</b><ul>${reasons}</ul></div>`:""}${issues?`<div class=ai-issues><h4>具体的な改善ポイント</h4>${issues}</div>`:"<p class=okbox>優先して直す具体的な誤りは見つかりませんでした。</p>"}${strengths?`<div class=ai-strengths><b>できている点</b><ul>${strengths}</ul></div>`:""}${feedback.revisedExample?`<details class=ai-revision><summary>あなたの答案を活かした改善例</summary><p>${h(feedback.revisedExample)}</p></details>`:""}<div class=ai-feedback-actions><button type=button class=primary onclick="applyAIGrade(${y},'${id}')" ${stale?"disabled":""}>この点数を自己採点欄に反映</button><small>AI使用量：入力 ${Number(feedback.usage?.inputTokens)||0} / 出力 ${Number(feedback.usage?.outputTokens)||0} tokens</small></div></section>`}
function aiWritingInput(y,q,key,entry){if(!AI_GRADING_QUESTIONS.has(key))return "";const answer=entry.answerText||"",feedback=aiFeedbackMarkup(y,q.id,entry);return `<section class=ai-grading-box><div class=ai-grading-title><b>AIで答案を詳しく確認（任意）</b><span>学習用</span></div><p class=tiny>答案を入力すると、内容・論理・文法・綴りを具体的に指摘します。点数は公式配点ではなく24点相当の学習用目安です。</p><textarea id="ai-answer-${y}-${q.id}" maxlength=1200 placeholder="紙に書いた英作文をここにも入力" oninput="rememberAIAnswer(${y},'${q.id}',this.value)">${h(answer)}</textarea><div class=ai-input-foot><span id="ai-word-count-${y}-${q.id}">${wordCount(answer)}語 / 目安50語</span><button type=button id="ai-grade-${y}-${q.id}" onclick="gradeWritingWithAI(${y},'${q.id}')">AIで学習用採点</button></div><div id="ai-status-${y}-${q.id}" class="tiny ai-status" role=status aria-live=polite></div>${feedback}</section>`}
function validateAIFeedback(data){return data&&Number.isInteger(data.score)&&data.score>=0&&data.score<=24&&Array.isArray(data.semantic)&&data.semantic.length===6&&Array.isArray(data.breakdown)&&Array.isArray(data.issues)}
async function gradeWritingWithAI(y,id){const key=k(y,id),entry=S.manual[key]||{score:"",components:[]},answer=String(entry.answerText||"").trim(),button=document.getElementById(`ai-grade-${y}-${id}`),status=document.getElementById(`ai-status-${y}-${id}`);if(!answer)return alert("AIで確認する英文答案を入力してください。");if(button)button.disabled=true;if(status)status.textContent="答案を確認しています…";try{const response=await fetch(`${AI_GRADING_API}/v1/grade-writing`,{method:"POST",headers:{"content-type":"application/json","x-client-id":aiClientId()},body:JSON.stringify({questionId:key,answer})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||(response.status===429?"利用が集中しています。少し時間をおいてください。":"AI採点を利用できませんでした。"));if(!validateAIFeedback(data))throw new Error("AI採点結果を安全に確認できませんでした。");const current=S.manual[key]||entry;S.manual[key]={...current,answerText:answer,aiSemantic:data.semantic,aiFeedback:{...data,answerFingerprint:aiAnswerFingerprint(answer)},aiFeedbackStale:false,gradingMode:current.gradingMode||"self",semanticRubricVersion:data.promptVersion,scorePolicyVersion:data.scorePolicyVersion,modelSnapshot:data.model};save();render()}catch(error){if(status)status.textContent=`${error.message} 自己採点はそのまま利用できます。`;if(button)button.disabled=false}}
function aiComponents(feedback){const [s1,s2,r1,r2,,g]=feedback.semantic||[],components=[];if(s1!==1||s2!==1)components.push("相手の主張の要約");if(Number(feedback.organization)>0)components.push("反論への移行");if(r1!==1)components.push("反論の明確さ");if(r2!==1)components.push("理由・具体例");if(Number(feedback.wordCount)<40||Number(feedback.wordCount)>60)components.push("語数・条件");if(g>0)components.push("文法・語彙");return components}
function applyAIGrade(y,id){const key=k(y,id),entry=S.manual[key],feedback=entry?.aiFeedback;if(!feedback||entry.aiFeedbackStale)return;S.manual[key]={...entry,score:feedback.score,components:[...new Set([...(entry.components||[]),...aiComponents(feedback)])],gradingMode:"ai-assisted"};save();render()}

...
function answerRow(y,q){
 ...
 if(q.type==="choice"){
   ...
   input=`<label class=input-guide for="a-${q.id}">${q.skill==="extract"?`本文から指定された${q.extractWords||1}語を抜き出して入力`:"英語の答えを入力"}</label><input id="a-${q.id}"${disabled} value="${h(val)}" placeholder="${placeholder}" autocomplete=off autocapitalize=none spellcheck=false oninput="rememberAnswer(${y},'${q.id}',this.value)">`;
 }else{
   const entry=S.manual[key]||{},score=entry.score??"";
   const selected=entry.components||[];
   const guide=window.MANUAL_GUIDES?.[key];
   input=`<div class=input-guide><b>記述問題</b>：紙に書いた答案を公式解答と比べ、自己採点した点数を入力</div>${guide?`<details class=official-guide><summary>${h(guide.title)}</summary><p class=official-answer>${h(guide.answer)}</p><p class=tiny>${h(guide.note)}</p></details>`:`<p class="tiny muted">この年度は公式解答冊子を手元で確認して採点してください。未確認の模範解答は表示しません。</p>`}${aiWritingInput(y,q,key,entry)}<div class=score-input><button type=button aria-label="1点減らす" onclick="adjustScore(${y},'${q.id}',-1,${q.points})">−</button><input id="m-${q.id}" type=number inputmode=numeric min=0 max=${q.points} value="${score}" placeholder="未採点" oninput="rememberScore(${y},'${q.id}',this.value,${q.points})"><span>/ ${q.points}点</span><button type=button aria-label="1点増やす" onclick="adjustScore(${y},'${q.id}',1,${q.points})">＋</button></div><details class=component-picker ${score!==""&&Number(score)<q.points?"needs-input":""}><summary>減点された原因を選ぶ</summary>${manualComponents(q).map(c=>`<label><input type=checkbox ${selected.includes(c)?"checked":""} onchange="rememberManualComponent(${y},'${q.id}','${c}',this.checked)"> ${c}</label>`).join("")}</details>`;
 }

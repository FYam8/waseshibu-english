...
function applyAIGrade(y,id){const key=k(y,id),entry=S.manual[key],feedback=entry?.aiFeedback,q=D[y]?.find(x=>x.id===id);if(!feedback||entry.aiFeedbackStale||!q)return;S.manual[key]={...entry,score:feedback.score,components:[...new Set([...(entry.components||[]),...aiComponents(feedback,q.skill)])],gradingMode:"ai-assisted"};save();render()}
...
async function gradeDrillWithAI(){const q=drillState?.q,task=drillWritingTask(q),answer=drillAIAnswer(q),button=document.getElementById("drill-ai-grade"),status=document.getElementById("drill-ai-status");if(!task)return;if(!answer||q.partLimits&&(drillState.selfParts||[]).some(x=>!String(x||"").trim()))return alert("AIで確認する答案をすべて入力してください。");if(button)button.disabled=true;if(status)status.textContent="答案を確認しています…";try{const data=await requestWritingFeedback(task,answer);drillState.aiFeedback={...data,answerFingerprint:aiAnswerFingerprint(answer)};drillState.aiFeedbackStale=false;persistDrill();render()}catch(error){if(status)status.textContent=error.message;if(button)button.disabled=false}}

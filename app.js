...
function exportData(){const payload={appId:"waseshibu-english-adaptive",schemaVersion:SCHEMA_VERSION,exportedAt:new Date().toISOString(),state:S};const blob=new Blob([JSON.stringify(payload,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`waseshibu-english-backup-${today()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
function validateImport(payload){if(!payload||payload.appId!=="waseshibu-english-adaptive"||!payload.state)throw new Error("このアプリのバックアップではありません。");if(Number(payload.schemaVersion)>SCHEMA_VERSION)throw new Error("このアプリより新しい形式のバックアップです。");const x=payload.state;if(!x.answers||!x.weak||!Array.isArray(x.attempts||[]))throw new Error("必要な学習データがありません。");for(const a of x.attempts||[]){const listeningInvalid=a.listeningScore!==null&&a.listeningScore!==undefined&&(Number(a.listeningScore)<0||Number(a.listeningScore)>20);if(!ROUTE.includes(Number(a.year))||Number(a.writtenScore)<0||Number(a.writtenScore)>80||listeningInvalid)throw new Error("年度または点数の範囲が不正です。")};return x}

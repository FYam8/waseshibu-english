...
const AI_GRADING_API="https://waseshibu-writing-grader.fyam8.workers.dev", AI_GRADING_CLIENT_KEY="waseshibu.ai-grading.client.v1";
...
function aiClientId(){try{let id=localStorage.getItem(AI_GRADING_CLIENT_KEY);if(!/^[a-zA-Z0-9._-]{12,100}$/.test(id||"")){id=`web-${crypto.randomUUID?.()||`${Date.now()}-${Math.random().toString(36).slice(2)}`}`;localStorage.setItem(AI_GRADING_CLIENT_KEY,id)}return id}catch{return `web-${Date.now()}-${Math.random().toString(36).slice(2)}`}}
...
async function requestWritingFeedback(task,answer){const response=await fetch(`${AI_GRADING_API}/v1/grade-writing`,{method:"POST",headers:{"content-type":"application/json","x-client-id":aiClientId()},body:JSON.stringify({task,answer})}),data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||(response.status===429?"利用が集中しています。少し時間をおいてください。":"AI採点を利用できませんでした。"));if(!validateAIFeedback(data,task.maxScore))throw new Error("AI採点結果を安全に確認できませんでした。");return data}
